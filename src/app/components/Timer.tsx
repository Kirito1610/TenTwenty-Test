...
function Timer({
  ...
  return (
    <div className=" flex  gap-8 relative">
      <div
        onClick={() =>
          onNext(currentActiveIndex + 1 > 3 ? 0 : currentActiveIndex + 1)
        }
        className="w-20 cursor-pointer border relative border-white p-2 h-20"
      >
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
          ...
          <div
            ...
            style={{
              width: `${active === 2 ? progress : active > 2 ? 100 : 0}%`,
            }}
          />
          <div
            className="absolute -top-[1px] -right-0.5 w-1 bg-white origin-top"
            style={{
              height: `${active === 1 ? progress : active > 1 ? 100 : 0}%`,
            }}
          />
        </div>
      ...
      <div className="flex items-center justify-between w-48 mx-auto">
        ...
        <div className="text-white  w-6 overflow-hidden text-lg">
          {Array.from({ length: 5 }).map((_, i) => (
            <p
              key={i}
              className={`inline-block h-8 overflow-hidden absolute transition-all duration-700 ${i === currentActiveIndex ? "-translate-y-3.5 opacity-100" : "translate-y-5 opacity-0"}`}
            >{`0${i + 1}`}</p>
          ))}
        </div>
